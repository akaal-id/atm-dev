...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    async signIn({ user, account }) {
      ...
      const users = await listResourceByFieldUnscoped("Users", "email", user.email.toLowerCase(), { limit: 1 });
    ...
    async session({ session }) {
      ...
      const users = await listResourceByFieldUnscoped("Users", "email", session.user.email.toLowerCase(), { limit: 1 });
