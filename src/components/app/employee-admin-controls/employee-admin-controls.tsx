...
import styles from "./employee-admin-controls.module.css";
    ...
    <div className={styles.group}>
      <form onSubmit={saveUser} encType="multipart/form-data" className={styles.form}>
        ...
        <div className={styles.region}>
          <Button type="submit" size="xl" className={styles.submit} disabled={saving}>
            {saving ? "Saving..." : "Save user"}
          </Button>
        </div>
      </form>

      {message ? <p className={styles.itemDescription}>{message}</p> : null}

      {canRemove ? (
        <Button type="button" variant="destructiveOutline" size="xl" className={styles.button} onClick={removeUser} disabled={removing}>
          <Trash2 className={styles.subheading} />
          {removing ? "Removing..." : "Remove user account"}
        </Button>
      ) : (
        <p className={styles.text}>You cannot remove your own account while signed in.</p>
      )}
    </div>
  );
}
