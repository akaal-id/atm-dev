...
import styles from "./email-blast-create-group-form.module.css";
...
export function EmailBlastCreateGroupForm({ busy = false, onCreate }: EmailBlastCreateGroupFormProps) {
  ...
  return (
    <form onSubmit={(event) => void handleSubmit(event)} className={styles.form}>
      ...
      {error ? <p className={styles.itemDescription}>{error}</p> : null}
      <Button type="submit" variant="default" size="xl" className={styles.button} disabled={busy}>
